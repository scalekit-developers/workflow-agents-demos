...
| Form responses not grouped correctly | Set `FORM_EMPLOYEE_QUESTION_ID` explicitly using the question ID from `googleforms_get_form`. Matching is currently **exact string match** against Airtable's `Employee Name` — a reviewer typing "Alex" instead of "Alex Kim" will be silently dropped |
