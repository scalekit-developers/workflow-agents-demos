...
All five steps run end-to-end against live PagerDuty, Jira, Confluence, and Slack workspaces: a page is triggered, a ticket is opened and cross-linked to it, an on-call channel is notified, and a postmortem doc is created from a template. Re-running with the same `--incident-key` is a safe no-op -- nothing is re-paged, re-ticketed, or re-notified.

Every step but Confluence is required for a successful run; a Confluence failure (most commonly an unresolved `CONFLUENCE_SPACE_ID`, see Prerequisites) is logged as a warning and does not fail the run, since the page, ticket, and notification are already real and actionable on their own.
...
- **A Jira account with a Jira connection set up in your Scalekit dashboard.** Scalekit's catalog has three separate Jira-related connectors (`JIRA`, `JIRASERVICEMANAGEMENT`, `ATLASSIANMCP`) -- this agent uses the plain `JIRA` REST connector. Copy the exact connection name into `JIRA_CONNECTOR`. Set `JIRA_PROJECT_KEY` to the project incidents should be filed under, and confirm `JIRA_ISSUE_TYPE` is a valid issue type name for that specific project -- issue type names vary per project, so check with `jira_issue_create_meta_issue_types_list` rather than assuming "Bug" exists everywhere.
- **A Confluence account with a Confluence connection set up in your Scalekit dashboard.** Uses the plain `CONFLUENCE` REST connector (not `ATLASSIANMCP`). Set `CONFLUENCE_SPACE_ID` to the numeric space ID postmortems should be created under -- not the space key (e.g. `SD`) and not your site's cloud ID (a UUID); Confluence's API rejects both with `"Provided value {...} for 'id' is not the correct type. Expected type is long"`. To find the numeric ID: open the space in your browser (its URL looks like `https://yoursite.atlassian.net/wiki/spaces/<KEY>/overview?homepageId=<PAGE_ID>`), then call `confluence_page_get(id=<PAGE_ID>)` -- the response's `spaceId` field is what `confluence_page_create` needs.
...
A few things about Jira and Confluence that aren't obvious from their docs and matter for setup:

**Jira has three connectors in Scalekit's catalog** -- `JIRA` (plain REST, general issue tracking), `JIRASERVICEMANAGEMENT` (customer requests and service desks, a different product), and `ATLASSIANMCP` (a unified MCP server covering Jira, Confluence, and Compass together). This agent uses the plain `JIRA` connector: it's the most direct match for "create an incident ticket," and unlike `ATLASSIANMCP` it doesn't require resolving a separate `cloudId` for every call.

**Confluence's plain REST connector has no spaces-list tool.** `confluence_page_create` requires a numeric `spaceId`, and there's no catalog tool that resolves one from a human-readable space key (only `ATLASSIANMCP`'s `atlassianmcp_getconfluencespaces` does, and this agent doesn't use that connector). See [Prerequisites](#prerequisites) for how to find your space's numeric ID.

**Jira ticket creation doesn't return a browsable URL.** The `self` field on a created issue only ever contains the API's cloud UUID (`https://api.atlassian.com/ex/jira/{cloud_id}/...`) -- Jira's real browse URL needs your site's subdomain, which no tool exposes. This agent never guesses at a URL from the UUID (a wrong guess is a broken link, which is worse than no link). By default, tickets are referenced by key alone (e.g. `ECS-18`) everywhere they're mentioned; set `JIRA_SITE_URL` to get clickable links in Slack, the PagerDuty note, and the Confluence doc instead.

| Tool | Method/Path | Notes |
|------|--------------|-------|
| `pagerduty_incident_create` | `POST /incidents` | Requires `title`, `service_id`, `from_email`. `incident_key`, if set, is PagerDuty's own server-side deduplication key -- this agent relies on it for idempotency at the source, not only its local state file. |
| `pagerduty_services_list` | `GET /services` | Resolves `PAGERDUTY_SERVICE_NAME` to the `service_id` the create tool requires. |
| `pagerduty_incident_note_create` | `POST /incidents/{id}/notes` | Posts the Jira ticket link back onto the PagerDuty incident. Non-fatal if it fails -- the incident and ticket already exist regardless. |
| `jira_issue_create` | `POST /ex/jira/{cloud_id}/rest/api/3/issue` | Requires `project_key`, `summary`, `issue_type`. `issue_type` and `priority_name` are project-specific names, not IDs -- an invalid name fails at Jira's own API with a message this agent surfaces as-is. |
...
| `slackmcp_slack_search_channels` | (MCP) | Resolves `SLACK_CHANNEL` from a bare name to a channel ID. Returns a markdown text block, not structured JSON -- the channel ID is parsed out of each result's `Permalink` URL. Requires an exact name match; there's no fuzzy fallback, since notifying the wrong channel is worse than failing to resolve one. |
| `slackmcp_slack_send_message` | (MCP) | `channel_id=`/`message=`, not `channel`/`text`. |
...
| `JIRA_SITE_URL` | (empty) | Optional: your Jira site URL (e.g. `https://yourteam.atlassian.net`), used to build a clickable ticket link. Left blank, tickets are referenced by key alone (e.g. `ECS-18`) -- see [Connector Notes](#connector-notes) for why this can't be discovered automatically |
...
| `Confluence postmortem creation failed ... "Could not create content with type page"` (logged as a warning, run still exits 0) | `CONFLUENCE_SPACE_ID` isn't a valid numeric space ID. See [Prerequisites](#prerequisites) for how to find it: open the space's homepage in your browser, note the `homepageId` query param in the URL, then call `confluence_page_get(id=<homepageId>)` and read its `spaceId` field |
